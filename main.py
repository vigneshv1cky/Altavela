    ...
    for pick in picks:
        try:
            ...
            if mid in _profit_exit_markets:
                evidence.append(f"[NOTE] This market recently had a profitable {_profit_exit_markets[mid]} exit. Consider mean reversion.")

                if pick["direction"] != _profit_exit_markets[mid]:
                    from altavela.desk.team import reversion_gate
                    gate = await loop.run_in_executor(
                        None, lambda: reversion_gate(
                            pick["question"], _profit_exit_markets[mid], pick["direction"], evidence))
                    if not gate.get("approve_reversion", False):
                        log.info("Reversion REJECTED: %s — %s", pick["question"][:60], gate.get("reason", ""))
                        continue
