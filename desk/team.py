...
# ---------------------------------------------------------------------------
# Plan — decides entry size and whether to execute
# ---------------------------------------------------------------------------

_PLAN_SYSTEM = (
    "You are the trading planner on a prediction-market desk. Your job: decide "
    "whether to execute a pick and at what size, given the debate result and "
    "portfolio state.\n\n"
    "Rules:\n"
    "  • approved: only approve if the judge's conviction is meaningful AND "
    "    portfolio limits allow it (respect max positions, category caps)\n"
    "  • size_fraction: 0.0 to 1.0 — how much of the max position size (${max_size}) to allocate\n"
    "  • STRONG verdict → 0.8–1.0, SOFT → 0.4–0.6, PASS → 0.1–0.2 if approved\n"
    "  • Deny the trade if: flipped direction, PASS with thin edge, too many "
    "    open positions in same category, or the end date is too far / too near\n\n"
    "  • reasoning: one sentence explaining your decision\n\n"
    'Return ONLY JSON: {{"approved": true|false, "size_fraction": 0.XX, "reasoning": "..."}}'
)

_PLAN_SCHEMA = {
    "approved": {"type": bool},
    "size_fraction": {"type": (int, float), "min": 0.0, "max": 1.0},
    "reasoning": {"type": str, "maxlen": 200},
}


def plan_entry(question: str, direction: str, verdict: str, approved: bool,
               adjusted_prob: float, adjusted_score: float, flipped: bool,
               end_date: str, category_positions: int, open_positions: int,
               max_positions: int, max_size: float,
               decision_id: str | None = None) -> dict:
    user = (
        f"Market: {question}\n"
        f"Direction: {direction}\n"
        f"Judge verdict: {verdict} (score {adjusted_score}/100, prob {adjusted_prob})\n"
        f"Judge approved: {approved}\n"
        f"Flipped direction: {flipped}\n"
        f"End date: {end_date}\n\n"
        f"Portfolio: {open_positions} open positions (max {max_positions}), "
        f"{category_positions} in this category\n"
        f"Max position size: ${max_size}"
    )
    out = call_role("plan", _PLAN_SYSTEM.format(max_size=max_size), user,
                    schema=_PLAN_SCHEMA, decision_id=decision_id)
    out.pop("_downgraded_model", None)
    return out
